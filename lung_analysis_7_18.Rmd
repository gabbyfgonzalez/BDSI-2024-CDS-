...
predictors <- modData1[, c("survival_days", "survival_status", "age_at_diagnosis", "gender_1", "p_Tumor", "p_Macrophage", "p_Cytotoxic_T", 
...
k_BCell_CytotoxicT_quantile_33 <- quantile(filtered_modData$k_BCell_CytotoxicT, probs = 0.33)
k_BCell_CytotoxicT_quantile_66 <- quantile(filtered_modData$k_BCell_CytotoxicT, probs = 0.66)


k_BCell_CytotoxicT_quantile_33
k_BCell_CytotoxicT_quantile_66


filtered_modData_surv <- filtered_modData %>%
  mutate(cluster = case_when(
    k_BCell_CytotoxicT < k_BCell_CytotoxicT_quantile_33 ~ "<33",
    k_BCell_CytotoxicT >= k_BCell_CytotoxicT_quantile_33 & k_BCell_CytotoxicT < k_BCell_CytotoxicT_quantile_66 ~ "33-66",
    k_BCell_CytotoxicT >= k_BCell_CytotoxicT_quantile_66 ~ ">66"
  ))
table(filtered_modData_surv$cluster)
table(filtered_modData_surv$cluster, filtered_modData_surv$stage_numeric)
boxplot(filtered_modData_surv$age_at_diagnosis ~ filtered_modData_surv$cluster)

# Check the age groups
filtered_modData_surv$cluster

# Kaplan-Meier Curve
km_fit <- survfit(
  Surv(survival_days, survival_status) ~ cluster, data = filtered_modData_surv)


kap_lung<-ggsurvplot(km_fit, 
           size=1.5,  # Use "Blues" palette
           title = "       Lung Cancer: Clustering B Cell and Cytotoxic T", 
           risk.table =F,
           #legend.labs = lab_names,
           xlab="Time (days)",
           legend.title="",
...
filtered_modData <- modData[complete.cases(modData2 %>%
                                             select(p_Tumor,
        p_Macrophage,
        p_Cytotoxic_T,
        p_T_Helper,
...
?scale
final_fit  = glmnet(x_standardized, y, alpha=0.05, lambda=lambda_min, family = "cox", standardize = FALSE)
final_coef = coef(final_fit, s = lambda_min)
final_coef

label = colnames(x)[which(abs(final_coef) > 0)]
...
library(tidyr)
library(plyr)
df$label<-revalue(df$label, c("p_Macrophage"="Proportion Macrophage",
                              "p_T_Helper"="Proportion T-Helper",
                              "p_B_Cell"="Proportion B Cell", 
                              "k_Cytotoxic_T"="K Cytotoxic T", 
                              "k_B_Cell"="K B Cell", 
                              "k_T_Helper"="K T-Helper",
                              "k_BCell_CytotoxicT" = "K B Cell & Cytotoxic T",
                              "k_BCell_THelper" = "K B Cell & T Helper",
                              "k_CytotoxicT_THelper" = "K Cytotoxic T & T Helper",
                              "k_Macrophage_THelper" = "K Macrophage & T Helper"
                          ))

df

treatment_levels <- c("Proportion Macrophage", "Proportion T-Helper", 
                      "Proportion B Cell", "K Cytotoxic T",
                      "K B Cell", "K T-Helper",
                      "K B Cell & Cytotoxic T","K B Cell & T Helper",
                      "K Cytotoxic T & T Helper",
                      "K Macrophage & T Helper"
                      )

flipped_levels <- rev(treatment_levels)

df$label <- factor(df$label, levels = flipped_levels)
names(df)
...
forest <- ggplot(df, aes(x = estimate, y = label, color = sign )) + 
  ...
  ggtitle("Lung: Spatial Features Selected") +
...
#remove rows from total data set
selected_filtered_modData <- modData2[complete.cases(modData2 %>%
                                             select(p_Macrophage,
        p_T_Helper,
        p_B_Cell,
         k_Cytotoxic_T,    
...
selected_filtered_modData

x_selected<-as.matrix(lung_k_selected)
x_selected

modData

#
y_selected <- selected_filtered_modData  %>%
  mutate(time=survival_days,
         status=survival_status) %>%
  select(time, status) %>%
  as.matrix()
...
k_BCell_CytotoxicT_quantile_0 <- 0

k_BCell_CytotoxicT_quantile_40 <- quantile(selected_filtered_modData$k_BCell_CytotoxicT, probs = 0.40)
k_BCell_CytotoxicT_quantile_40 <- 100000
k_BCell_CytotoxicT_quantile_50 <- quantile(filtered_modData$k_BCell_CytotoxicT, probs = 0.50)
k_BCell_CytotoxicT_quantile_75 <- quantile(selected_filtered_modData$k_BCell_CytotoxicT, probs = 0.75)
...
selected_newData <- with(filtered_modData, data.frame(
 ...
 k_BCell_CytotoxicT = c(k_BCell_CytotoxicT_quantile_0,k_BCell_CytotoxicT_quantile_75),
...
selected_cox_fit<- coxph(Surv(survival_days, survival_status) ~ age_at_diagnosis +
    stage_new3_plus +
    gender_1+
        p_Macrophage+
        p_T_Helper+
        ...
        k_B_Cell+
         ...
         k_BCell_THelper+
         k_CytotoxicT_THelper+
         k_Macrophage_THelper, data = selected_filtered_modData)


summary(selected_cox_fit)
final
# Fit survival curves using survfit
fit <- survfit(selected_cox_fit, newdata = selected_newData)
summary(fit)


# Plot survival curves using ggsurvplot
surv_plot <- ggsurvplot(fit, data = selected_newData,
                        conf.int = T,
                        palette =
    c("darkred", "navy"),
                        legend.labs = c("25", "75"),
    ...
    font.x        = c(15, "bold"),
    font.y        = c(15, "bold"),
    font.xtickslab = c(13, "bold"),
    font.ytickslab = c(13, "bold"),
     title    = "Lung Data: Proportion of Macrophages",
...
p_b_cells10 <- quantile(filtered_modData$p_B_Cell, probs = 0.05)
p_b_cells_90 <- quantile(filtered_modData$p_B_Cell, probs = 0.95)


p_macrophage_quantile_.1 <- quantile(filtered_modData$p_Macrophage, probs = 0.1)
p_macrophage_quantile_.9 <- quantile(filtered_modData$p_Macrophage, probs = 0.9)
p_macrophage_quantile_25

p_macrophage_quantile_50 <- quantile(filtered_modData$p_Macrophage, probs = .9)


boxplot(filtered_modData$p_Macrophage)



newData_p_macrophage <- with(filtered_modData, data.frame(
  age_at_diagnosis = mean(age_at_diagnosis, na.rm = TRUE),
  stage_new2 = 0,
  stage_new3_plus = 0,
  gender_1 = 0,
  p_Tumor = mean(p_Tumor, na.rm = TRUE),
  p_Macrophage =  c(p_macrophage_quantile_.1, p_macrophage_quantile_.9),
  p_Cytotoxic_T = mean(p_Cytotoxic_T, na.rm = TRUE),
  p_T_Helper = mean(p_T_Helper, na.rm = TRUE),
  p_B_Cell = c(p_b_cells10, p_b_cells_90),
  K_Tumor = mean(k_Tumor, na.rm = TRUE),
  k_Cytotoxic_T = mean(k_Cytotoxic_T, na.rm = TRUE),
   k_B_Cell = mean(k_B_Cell, na.rm = TRUE),
...
newData_p_macrophage<-as.matrix(newData_p_macrophage)
newData_p_macrophage

y_fit<-Surv(filtered_modData$survival_days, filtered_modData$survival_status)


survFIT_p_b_cells<-survival::survfit(cross_mod, s = lambda_min, x = x, y = y_fit, newx = newData_p_macrophage)

summary(survFIT)

ggsurvplot(survfit_obj,
           data=x_ov, 
           ...
           font.xtickslab=c(13,"bold"), 
           font.ytickslab = c(13,"bold"), 
           title = "Ovarian Data: Clustering of Tumor Cells", 
           legend.labs = c("Low Clustering", "High Clustering"),
           ggtheme = theme_bw())


surv_plot_p_b_cells<-ggsurvplot(survFIT_p_b_cells,
           data=newData_p_macrophage,
           palette = c("red","darkblue"),
     xlab = "Time (days)",
    font.title    = c(18, "bold", "black"),
    ggtheme = theme_bw(),
    ...
    font.xtickslab = c(13, "bold"),
    font.ytickslab = c(13, "bold"),
     title    = "Lung Data: Proportion Macrophage",
    legend.labs =
    c("Low Proportion", "High Proportion"))
...
aft_fit_plot <- penAFT(x, ydf$time, ydf$status, 
                           penalty = NULL,
                       nlambda=50,
                           alpha=0.05,
                           lambda = NULL,
...
install.packages("aftreg")
library(aftreg)
library(survival)  # For working with survival data
...
#make new datasets
boxplot_dat_l <- filtered_modData%>%
select(p_T_Helper,
         k_Cytotoxic_T, 
         k_T_Helper,
           k_CytotoxicT_Macrophage)%>%
...
#make new dataset
boxplot_dat_o <- filtered_modData_ovarian %>%
  select(p_T_Helper,
         k_Cytotoxic_T, 
         k_T_Helper,
           k_CytotoxicT_Macrophage) %>%
  mutate(cancer_type = "Ovarian")

names(boxplot_dat_l)
names(boxplot_dat_o)


result<-rbind(boxplot_dat_l, boxplot_dat_o)

result_p_T_Helper <-result %>%
  select(cancer_type, p_T_Helper)

result_k_Cytotoxic_T <-result %>%
  select(cancer_type, k_Cytotoxic_T)

result_k_T_Helper <-result %>%
  select(cancer_type, k_T_Helper)

result_k_CytotoxicT_Macrophage <-result %>%
  select(cancer_type, k_CytotoxicT_Macrophage)


long_data_p_T_Helper <- result_p_T_Helper %>%
  pivot_longer(cols = -c(cancer_type),
               names_to = "cell_type",
               values_to = "value")

long_data_k_Cytotoxic_T <- result_k_Cytotoxic_T %>%
  pivot_longer(cols = -c(cancer_type),
               names_to = "cell_type",
               values_to = "value")


long_data_k_T_Helper <- result_k_T_Helper%>%
  pivot_longer(cols = -c(cancer_type),
               names_to = "cell_type",
               values_to = "value")

long_data_k_CytotoxicT_Macrophage <- result_k_CytotoxicT_Macrophage%>%
  pivot_longer(cols = -c(cancer_type),
               names_to = "cell_type",
               values_to = "value")
...
boxplot_prop_t_helper<-ggplot(long_data_p_T_Helper, aes(x = cell_type, y = value, fill = cancer_type)) +
  geom_boxplot() +
  scale_fill_manual(values = c("Ovarian" = "indianred2", "Lung" = "steelblue2")) +
  labs(title = "Proportion of T-Helper Cells",
       x = "",
       y = "Proportion",
       fill = "Cancer Type") +
    labs(fill="") +
  ...
  theme(plot.title = element_text(hjust = 0.5, face="bold", size=22),
        ...
        axis.text = element_text(size = 17, color="black"))+
  guides(fill = guide_legend(title = "", keywidth = 1.5, keyheight = 10, 
                             title.position = "top"))

boxplot_prop_t_helper
boxplot_k_Cytotoxic_T<-ggplot(long_data_k_Cytotoxic_T, aes(x = cell_type, y = value, fill = cancer_type)) +
  geom_boxplot() +
  scale_fill_manual(values = c("Ovarian" = "indianred2", "Lung" = "steelblue2")) +
  labs(title = "K Cytotoxic T",
       x = "",
       y = "K theoretical - K observed",
       fill = "Cancer Type") +
    labs(fill="") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5, face="bold", size=22),
...
boxplot_k_Cytotoxic_T


###---
boxplot_k_T_Helper<-ggplot(long_data_k_T_Helper, aes(x = cell_type, y = value, fill = cancer_type)) +
  geom_boxplot() +
  scale_fill_manual(values = c("Ovarian" = "indianred2", "Lung" = "steelblue2")) +
  labs(title = "K T-Helper",
       x = "",
       y = "K theoretical - K observed",
       fill = "Cancer Type") +
    labs(fill="") +
...
###---
boxplot_k_CytotoxicT_Macrophage<-ggplot(long_data_k_CytotoxicT_Macrophage, aes(x = cell_type, y = value, fill = cancer_type)) +
  geom_boxplot() +
  scale_fill_manual(values = c("Ovarian" = "indianred2", "Lung" = "steelblue2")) +
  labs(title = "K Cytotoxic T & Macrophage",
       x = "",
       y = "K theoretical - K observed",
       fill = "Cancer Type") +
    labs(fill="") +
  ...
  theme(plot.title = element_text(hjust = 0.5, face="bold", size=22),
        ...
        axis.title.y = element_text(size = 17, face = "bold"),
        axis.text = element_text(size = 17, color="black"))+
  guides(fill = guide_legend(title = "", keywidth = 5, keyheight = 8, 
                             title.position = "top"))

boxplot_k_CytotoxicT_Macrophage

library(cowplot)

#align plots
align_vcl_veloc<-plot_grid(boxplot_prop_t_helper, 
                           boxplot_k_Cytotoxic_T,
                           boxplot_k_T_Helper,
                           boxplot_k_CytotoxicT_Macrophage,
                           ncol=2, align="v")
...
title <- ggdraw() +
  draw_label("Pan-Cancer Features", fontface = "bold", size = 28)

#add white background to title
final_plot_hormone <- grid.arrange(
  title,
  align_vcl_veloc,
  ncol = 1,
  heights = c(0.1, 1)
...
ggsave(plot = final_plot_hormone, filename = "Pan_Cancer_Features.png", 
       dpi = 300, width = 17, height = 8)

longdata

names(result_k_Cytotoxic_T)
hist(result_k_Cytotoxic_T$k_Cytotoxic_T)

ggplot(long_data_k_CytotoxicT_Macrophage, aes(x = value, fill = cancer_type)) +
   geom_histogram(position="identity", alpha=0.5)+ # Adjust transparency with alpha
  labs(title = "k_CytotoxicT_Macrophage",
       x = "K observed - K expected",
       y = "Density") +
  scale_fill_manual(values = c("Lung" = "skyblue", "Ovarian" = "salmon")) +  # Custom fill colors
  theme_classic()

ggplot(long_data_p_T_Helper, aes(x = value, fill = cancer_type)) +
   geom_histogram(position="identity", alpha=0.5)+  # Adjust transparency with alpha
  labs(title = "p_T_Helper",
       x = "K observed - K expected",
       y = "Density") +
  scale_fill_manual(values = c("Lung" = "skyblue", "Ovarian" = "salmon")) +  # Custom fill colors
  theme_classic()

ggplot(long_data_k_Cytotoxic_T, aes(x = value, fill = cancer_type)) +
   geom_histogram(position="identity", alpha=0.5)+ # Adjust transparency with alpha
  labs(title = "k_Cytotoxic_T",
       x = "K observed - K expected",
       y = "Density") +
  scale_fill_manual(values = c("Lung" = "skyblue", "Ovarian" = "salmon")) +  # Custom fill colors
  theme_classic()

ggplot(long_data_k_T_Helper, aes(x = value, fill = cancer_type)) +
   geom_histogram(position="identity", alpha=0.5)+  # Adjust transparency with alpha
  labs(title = "k_T_Helper",
       x = "K observed - K expected",
       y = "Density") +
  scale_fill_manual(values = c("Lung" = "skyblue", "Ovarian" = "salmon")) +  # Custom fill colors
  theme_classic()

long_data_k_CytotoxicT_Macrophage
